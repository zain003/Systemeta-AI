  ...
  return (
    <div className="space-y-1 px-3 py-3">
      {projects.map((project) => (
        <div className="group flex items-center gap-3 rounded-xl px-3 py-2 hover:bg-subtle" key={project.id}>
    ...
    <>
      ...
      <Tabs defaultValue="my-projects" className="flex min-h-0 flex-1">
        ...
        <TabsContent value="my-projects" className="min-h-0 flex-1 overflow-auto">
          <ProjectList projects={projects.filter((project) => project.access === "owned")} onRenameProject={onRenameProject} onDeleteProject={onDeleteProject} />
        </TabsContent>
        <TabsContent value="shared" className="min-h-0 flex-1 overflow-auto">
          <ProjectList projects={projects.filter((project) => project.access === "shared")} onRenameProject={onRenameProject} onDeleteProject={onDeleteProject} />
        </TabsContent>
      </Tabs>
